...
func main() {
	...
	// Get all milestones
	milestones, err := getAllMilestones(ctx, client)
	if err != nil {
		log.Fatalf("Failed to fetch milestones: %v", err)
	}
	...
	if *issueNum > 0 {
		...
		if err != nil {
			log.Fatalf("Failed to fetch issue #%d: %v", *issueNum, err)
		}
		if issue.GetState() != "closed" {
			log.Fatalf("Issue #%d is not closed", *issueNum)
		}
		issues = []*github.Issue{issue}
	} else {
		issues, err = getAllClosedIssues(ctx, client)
		if err != nil {
			log.Fatalf("Failed to fetch closed issues: %v", err)
		}
	}
...
func processIssues(ctx context.Context, client *github.Client, issues []*github.Issue, milestones []*github.Milestone, rules []MilestoneRule, dryRun bool, verbose bool) {
	...
	for _, issue := range issues {
		...
		if !dryRun {
			...
			if err != nil {
				fmt.Printf("  ERROR: Failed to update issue #%d: %v\n", issue.GetNumber(), err)
				errors++
				continue
			}
...
func getAllMilestones(ctx context.Context, client *github.Client) ([]*github.Milestone, error) {
...
func getAllClosedIssues(ctx context.Context, client *github.Client) ([]*github.Issue, error) {
