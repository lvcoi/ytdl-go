...
describe('Header', () => {
    it('renders the title based on activeTab', () => {
        renderWithRouter(() => <Header activeTab="dashboard" />);
    ...
    it('toggles Advanced Mode style based on prop', () => {
        const { unmount } = renderWithRouter(() => <Header activeTab="dashboard" isAdvanced={true} />);
        const button = screen.getByText('Advanced Mode').closest('button');
        expect(button).toHaveClass('bg-accent-primary/20');
        unmount();

        renderWithRouter(() => <Header activeTab="dashboard" isAdvanced={false} />);
    ...
    it('calls onToggleAdvanced when button is clicked', () => {
        ...
        renderWithRouter(() => <Header activeTab="dashboard" onToggleAdvanced={toggleMock} />);
